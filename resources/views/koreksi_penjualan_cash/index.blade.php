...
<tbody></tbody>
  ...
  <div class="col-md-4">
    <form class="form form-horizontal form-penjualan" method="post" action="{{route('koreksi_penjualan_cash.simpan')}}">
      {{ csrf_field() }}
      <input type="hidden" name="idpenjualan" value="{{ $idpenjualan }}">
...
@include('koreksi_penjualan_cash.produk')
@endsection
...
$(function(){
  ...
  table = $('.tabel-penjualan').DataTable({
    ...
    "dom" : 'Brt',
    "ajax" : {
       "url" : "{{ route('koreksi_penjualan_cash.data', $idpenjualan) }}",
       "type" : "GET"
     }
  }).on('draw.dt', function(){
    loadForm($('#diskon').val());
...
function addItem(){
  $.ajax({
    url : "{{ route('koreksi_penjualan_cash.store') }}",
    type : "POST",
...
function changeCount(id){
  url = "{{route('koreksi_penjualan_cash.update',':id')}}"
...
function deleteItem(id){
  if(confirm("Apakah yakin data akan dihapus?")){
    ...
    url = "{{route('koreksi_penjualan_cash.destroy',':id')}}"
    url = url.replace(':id',id);
...
function loadForm(diskon=0, diterima=0){
  ...
  url = "{{route('koreksi_penjualan_cash.loadform',[':diskon',':total',':diterima',$idpenjualan])}}"

...
<tbody></tbody>
  ...
  <div class="col-md-4">
    <form class="form form-horizontal form-pembelian" method="post" action="{{  route('kirim_antar_gudang.store') }} ">
      {{ csrf_field() }}
      <input type="hidden" name="idpembelian" value="{{ $idpembelian }}">
      ...
      <div class="pull-right">
        ...
        <a href="{{route('kirim_antar_gudang.index')}}" class="btn btn-warning"><i class="fa fa-floppy-o"></i> Hold</a> 
      </div>
      </div>
    </div>
  </div>
...
@include('kirim_antar_gudang_detail.produk')
@endsection
...
"paging": false,
     "ajax" : {
       "url" : "{{ route('kirim_antar_gudang_detail.data', $idpembelian) }}",
       "type" : "GET"
     }
  }).on('draw.dt', function(){
...
function addItem(){
  $.ajax({
    url : "{{ route('kirim_antar_gudang_detail.store') }}",
    type : "POST",
...
function changeCount(id){
  var url = "{{route('kirim_antar_gudang_detail.update',':id')}}";
...
function changeExpired(id){
  var url = "{{route('kirim_antar_gudang_detail.update_expired',':id')}}";
  url = url.replace(':id', id);
     $.ajax({
        url : url,
        type : "GET",
...
function deleteItem(id){
  ...
  var url = "{{route('kirim_antar_gudang_detail.destroy',':id')}}";
  url = url.replace(':id', id);
...
function loadForm(id){
  var url = "{{route('kirim_antar_gudang_detail.loadForm',':id')}}";
  url = url.replace(':id', id);
  $.ajax({
       url : url,
       type : "GET",

...
<tbody></tbody>
  ...
  <div class="col-md-4">
    <form class="form form-horizontal form-penjualan" method="post" action="{{route('koreksi_penjualan.simpan')}}">
      {{ csrf_field() }}
      <input type="hidden" name="idpenjualan" value="{{ $idpenjualan }}">
      ...
      <div class="box-footer">
        ...
        <a href="{{route('koreksi_penjualan.batal',$idpenjualan)}}" class="btn btn-warning pull-right" style="margin-right: 5px;" > Batal Transaksi</a>
      </div>
    </div>
  </div>
</div>
@include('koreksi_penjualan.produk')
...
$(function(){
  ...
  table = $('.tabel-penjualan').DataTable({
    ...
    "paging": false,
    "ajax" : {
      "url" : "{{ route('koreksi_penjualan.listDetail', $idpenjualan) }}",
      "type" : "GET"
    }
  }).on('draw.dt', function(){
    loadForm($('#diskon').val());
...
function addItem(){
  $.ajax({
    url : "{{ route('koreksi_penjualan.store') }}",
    type : "POST",
...
function changeCount(id){
  ...
  url = "{{route('koreksi_penjualan.update',':id')}}"
...
function deleteItem(id){
  if(confirm("Apakah yakin data akan dihapus?")){
    ...
    url = "{{route('koreksi_penjualan.destroy',':id')}}"
    url = url.replace(':id',id);
...
function loadForm(diskon=0, diterima=0){
  ...
  url = "{{route('koreksi_penjualan.loadform',[':diskon',':total',':diterima',$idpenjualan])}}"

...
@include('approve_retur_supplier.detail')
@endsection
...
$(function(){
   table = $('.tabel-retur').DataTable({
     ...
     "serverside" : true,
     "ajax" : {
       "url" : "{{ route('approve_retur_supplier.data') }}",
       "type" : "GET"
     }
   }); 
...
function showDetail(id){
  ...
  url = "{{route('approve_retur_supplier.show',':id')}}";
  url = url.replace(':id',id)
  
  url_approve = "{{route('approve_retur_supplier.approve',':id')}}"
  url_approve = url_approve.replace(':id',id)

  url_reject = "{{route('approve_retur_supplier.reject',':id')}}"

...
@include('approve_retur_tukar_barang.detail')
@endsection
...
$(function(){
   table = $('.tabel-retur').DataTable({
     ...
     "serverside" : true,
     "ajax" : {
       "url" : "{{ route('approve_retur_tukar_barang.data') }}",
       "type" : "GET"
     }
   }); 
...
function showDetail(id){
  ...
  url = "{{route('approve_retur_tukar_barang.show',':id')}}";
  url = url.replace(':id',id)
  
  url_approve = "{{route('approve_retur_tukar_barang.approve',':id')}}"
  url_approve = url_approve.replace(':id',id)

  url_reject = "{{route('approve_retur_tukar_barang.reject',':id')}}"

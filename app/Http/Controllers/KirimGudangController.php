...
class KirimGudangController extends Controller
{
    public function kirim_view(){
        $dataTransfer = Transfer::leftJoin('branch','transfer.id_user','=','branch.kode_toko')
                    ...
                    ->get();
        $no = 1;
        return view('transfer/gudang/kirim',['dataTransfer'=>$dataTransfer,'no'=>$no]);
    }
    public function kirim_detail($id){
            ...
            $nomer = 1;
            return view('transfer/gudang/kirim_detail',['transfer'=>$detail,'nomer'=>$nomer,'no_surat'=>$no_surat]);
    }
}
